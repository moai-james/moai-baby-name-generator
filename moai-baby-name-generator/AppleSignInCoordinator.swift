    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            guard let idTokenString = String(data: appleIDToken, encoding: .utf8) else {
                ...
                completion?(.failure(NSError(domain: "AppleSignIn", code: -1, userInfo: [NSLocalizedDescriptionKey: "Unable to serialize token string"])))
                return
            }
            ...
            Auth.auth().signIn(with: credential) { [weak self] (authResult, error) in
                ...
                if let authResult = authResult {
                    ...
                    self?.completion?(.failure(NSError(domain: "AppleSignIn", code: -1, userInfo: [NSLocalizedDescriptionKey: "Unknown error occurred"])))
                }
            }
        }
    }
